...
void Top_k::insert(string s){
	...
	if ( !heap.full() && !hash.exists_in_heap(s)){		
        ...
        int index = hash.getIndexAtString(s);
        int occ = heap.get_occurances_at_index(index);
        heap.set_occurances_at_index(index, occ+1); 
        //int end_index = heap.percolate_down(index);
        //swap_indicies_after_percolate(index, end_index);
        //hash.setIndexAtString(s, index);    
        //hash.setIndexAtString(s, end_index);
        return;
	}
		...
		heap.set_occurances_at_index(index_of_s, occurances+1);
